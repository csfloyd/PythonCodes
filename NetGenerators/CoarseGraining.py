...
cg2 = community.greedy_modularity_communities(G)
...
for key in size_list:
    sl.append(size_list[key]*10)


pos=nx.circular_layout(GCG)
nx.draw_networkx(GCG,pos,node_size = sl)
all_weights = []
    #4 a. Iterate through the graph nodes to gather all the weights
for (node1,node2,data) in GCG.edges(data=True):
    all_weights.append(data['weight']) #we'll use this when determining edge thickness
...
for weight in unique_weights:
        #4 d. Form a filtered list with just the weight you want to draw
    weighted_edges = [(node1,node2) for (node1,node2,edge_attr) in GCG.edges(data=True) if edge_attr['weight']==weight]
        #4 e. I think multiplying by [num_nodes/sum(all_weights)] makes the graphs edges look cleaner
    width = weight/10
    nx.draw_networkx_edges(GCG,pos,edgelist=weighted_edges,width=width)
